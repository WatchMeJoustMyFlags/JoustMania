            ...
            try:
                ...
                def play_thread():
                    ...
                    try:
                        ...
                        device.volume = volume  # Set volume for this sound
...
class AudioManager:
    ...
    def play_sound(self, file_path: str, volume: float = 1.0, priority: int = 2) -> bool:
        ...
        Args:
            ...
            volume: Volume level (0.0 to 1.0) - individual sound volume
        ...
        try:
            ...
            if channel:
                # Apply individual volume * SFX master volume
                adjusted_volume = volume * self.sfx_volume
    ...
    def set_music_volume(self, volume: float) -> bool:
        ...
        try:
            ...
            self.music_player.set_volume(self.music_volume)
            logger.debug(f"Music volume set to {self.music_volume:.2f}")
            return True
        except Exception as e:
    ...
    def set_sfx_volume(self, volume: float) -> bool:
        """
        Set master volume for sound effects only.
        ...
        try:
            ...
            logger.debug(f"SFX volume set to {self.sfx_volume:.2f}")
            return True
        except Exception as e:
            logger.error(f"Error setting SFX volume: {e}", exc_info=True)
            return False
    ...
    def set_volume(self, volume: float) -> bool:
        ...
        try:
            ...
            self.music_player.set_volume(self.music_volume)
            logger.debug(f"Master volume set to {volume:.2f}")
            return True
        except Exception as e:
    ...
    async def PlaySound(self, request, context):  # noqa: N802, ARG002
        ...
        with tracer.start_as_current_span(f"PlaySound:{sound_name}") as span:
            ...
            if not self.audio_enabled:
                ...
                logger.debug(f"Audio disabled, skipping sound: {sound_name}")
                return audio_pb2.PlaySoundResponse(success=True)
            
            # Play the sound with individual volume
            success = self.audio_manager.play_sound(
                resolved_path,
                volume=request.volume if request.volume > 0 else 1.0,
                priority=request.priority,
            )
            
            span.set_attribute("audio.success", success)
            return audio_pb2.PlaySoundResponse(success=success)
